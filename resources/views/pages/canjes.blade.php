    ...
    @if(auth()->guard('colaborador')->check())
        <div style="text-align: center; margin: 15px 0;">
            <span style="background: #ecfdf5; color: #10b981; padding: 8px 20px; border-radius: 30px; font-weight: 700; font-size: 1.05rem; border: 1px solid #a7f3d0;">
                Tus Puntos: {{ number_format(auth()->guard('colaborador')->user()->puntos_acumulados, 0, '', '.') }} pts
            </span>
        </div>
    @endif
    ...
    <section class="productos">
        ...
        @else
            @foreach($productos as $producto)
                <div class="producto" onclick="abrirModal('{{ $producto->id }}', '{{ $producto->nombre }}', '{{ $producto->costo_puntos }}')">
                    <div class="circulo">
                        @if($producto->imagen)
                    ...
                    <p class="puntos">{{ number_format($producto->costo_puntos, 0, '', '.') }} puntos</p>
                </div>
            @endforeach
        @endif
    </section>
...
<script>
    function abrirModal(id, nombre, puntos) {
        const estaLogueado = @json(auth()->guard('colaborador')->check());
